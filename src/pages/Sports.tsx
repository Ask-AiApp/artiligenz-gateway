...
const Sports = () => {
  ...
  useEffect(() => {
    const fetchSports = async () => {
      try {
        setLoading(true);
        const { data, error } = await supabase.functions.invoke('fetch-rss?category=sports');
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-sky-950">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <CategoryHero
          icon={Trophy}
          title="Sports (AI in Sports Analytics & Performance)"
          subtitle="AI innovations in sports analytics and athlete performance"
        />
